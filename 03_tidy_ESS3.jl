...
ESS3_partnered = @subset(ESS3_relations_long, :rship .== 1)

oppsex = Vector{Union{Int,Missing}}(undef, nrow(ESS3_partnered))

for i in 1:nrow(ESS3_partnered)
    local gndr = ESS3_partnered.gndr_1[i]
    local gndr_sp = ESS3_partnered.gndr[i]
...
ESS3_partnered[!, :oppsex] = oppsex

# Identify those with multiple husband/wife/partner relationships
ESS3_multipartner = @chain ESS3_partnered begin
    @groupby(:pid)
...
# Remove duplicates
ESS3_partnered = @chain ESS3_partnered begin
...
leftjoin!(ESS3, ESS3_partnered, on=:pid)
...
mstat = Vector{Union{String,Missing}}(undef, nrow(ESS3))

for i in 1:nrow(ESS3)
    local lvghwa = coalesce(ESS3.lvghwa[i], 99)
    local lvgptna = coalesce(ESS3.lvgptna[i], 99)
