...
ESS4_partnered = @subset(ESS4_relations_long, :rship .== 1)

oppsex = Vector{Union{Int,Missing}}(undef, nrow(ESS4_partnered))

for i in 1:nrow(ESS4_partnered)
    local gndr = ESS4_partnered.gndr_1[i]
    local gndr_sp = ESS4_partnered.gndr[i]
...
ESS4_partnered[!, :oppsex] = oppsex

# Identify those with multiple husband/wife/partner relationships
ESS4_multipartner = @chain ESS4_partnered begin
    @groupby(:pid)
...
# Remove duplicates
ESS4_partnered = @chain ESS4_partnered begin
...
leftjoin!(ESS4, ESS4_partnered, on=:pid)
...
mstat = Vector{Union{String,Missing}}(undef, nrow(ESS4))

for i in 1:nrow(ESS4)
    local lvghwa = coalesce(ESS4.lvghwa[i], 99)
    local lvgptna = coalesce(ESS4.lvgptna[i], 99)
