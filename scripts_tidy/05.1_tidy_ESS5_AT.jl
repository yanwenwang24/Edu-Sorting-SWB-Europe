...
ESS5_AT_partnered = @subset(ESS5_AT_relations_long, :rship .== 1)

oppsex = Vector{Union{Int,Missing}}(undef, nrow(ESS5_AT_partnered))

for i in 1:nrow(ESS5_AT_partnered)
    local gndr = ESS5_AT_partnered.gndr_1[i]
    local gndr_sp = ESS5_AT_partnered.gndr[i]
...
ESS5_AT_partnered[!, :oppsex] = oppsex

# Identify those with multiple husband/wife/partner relationships
ESS5_AT_multipartner = @chain ESS5_AT_partnered begin
    @groupby(:pid)
...
leftjoin!(
    ESS5_AT_partnered,
    ESS5_AT_multipartner,
    on=:pid,
    makeunique=true
)

# Remove duplicates
ESS5_AT_partnered = @chain ESS5_AT_partnered begin
...
leftjoin!(ESS5_AT, ESS5_AT_partnered, on=:pid)
